...
const forceTeacherDemo=new URLSearchParams(location.search).get('demo')==='teacher';
...
function openDetail(id){
  ...
  const imageSrc=r.demoImage||r.hasFile&&r.kind==='image'&&`/api/memories/${r.id}/file`;
  const imageAlt=r.demoImage?r.title:'保存的原始图片';
  const media=imageSrc?`<button type="button" class="mediaZoom" data-zoom="${esc(imageSrc)}" data-zoom-alt="${esc(imageAlt)}" aria-label="放大查看 ${esc(imageAlt)}"><img class="media" src="${esc(imageSrc)}" alt="${esc(imageAlt)}"></button>`:r.demoAudio?`<audio controls src="${esc(r.demoAudio)}"></audio>`:r.hasFile&&r.kind==='audio'?`<audio controls src="/api/memories/${r.id}/file"></audio>`:'';
